...
export class UpstreamManager {
  ...
  constructor(private readonly repo: GatewayRepository, private readonly db: Database, private readonly logger: { warn: (...args: unknown[]) => void; error: (...args: unknown[]) => void }) {}
  ...
  async refreshAll(): Promise<void> {
    const servers = await this.repo.listServers();
    await Promise.all(servers.filter((server) => server.enabled).map((server) => this.refreshServerTools(server.id).catch((error) => this.logger.warn({ err: error, serverId: server.id }, 'tool refresh failed'))));
  }
  ...
  async callTool(tool: ToolMetadata, args: unknown, signal?: AbortSignal): Promise<McpCallResult> {
    ...
    if (!connector) throw new Error('UPSTREAM_NOT_CONNECTED');
    return connector.callTool(tool.upstreamName, args, signal, tool.timeoutMs ?? config.defaultCallTimeoutMs);
  }
